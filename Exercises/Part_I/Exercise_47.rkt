...
(define HAPPINESS-DECREASE-BY-SECOND 1.0)
(define HAPPINESS-SMALL-INCREASE (/ 1 5)) ; 1/5 of the bar
(define HAPPINESS-BIG-INCREASE (/ 1 3))   ; 1/3 of the bar
...
(check-expect (increase-happiness 100 "down") (+ (/ HAPPINESS-MAX-LVL 5) 100))
(check-expect (increase-happiness 0 "down")  (/ HAPPINESS-MAX-LVL 5))
(check-expect (increase-happiness 150 "up") (+ (/ HAPPINESS-MAX-LVL 3) 150))
(check-expect (increase-happiness 0 "up") (/ HAPPINESS-MAX-LVL 3))
(check-expect (increase-happiness 100 "right") 100)
(define (increase-happiness ws key-event)
  (cond
    [(< ws HAPPINESS-MAX-LVL)   (cond
    
    [(key=? key-event "down") (+ (* HAPPINESS-MAX-LVL HAPPINESS-SMALL-INCREASE) ws)] ; increase happiness by 1/5 of max score on down arrow key
    [(key=? key-event "up") (+ (* HAPPINESS-MAX-LVL HAPPINESS-BIG-INCREASE) ws)]     ; increase happiness by 1/3 of max score on up arrow key
    [else ws]) 
                  ]
    [else ws])
  )
